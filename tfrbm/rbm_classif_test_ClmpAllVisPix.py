...
for j in range(n_data) :
    ...
    img = np.concatenate((t1, image.flatten()), axis=0)
    img_org = img
    ...
    for i in range(200):
        ...
        ## concatenate reconstructed labels & original image. img will be used for next reconstruction iteration
        img = np.concatenate((image_rec1[0:10], image), axis=0)
