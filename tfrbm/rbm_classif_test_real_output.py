...
for j in range(n_data) :
    ...
    ak = 0
    for i in range(200):
        if(i==199): #skip sampling at for the last returned data
            ak = 1
            #print("ak is ", ak)
            #print("i ", i)
        else:
        ...
        image_rec1 = bbrbm.reconstruct(img.reshape(1,-1),ak)

        #print("shape of of rec1",image_rec1.shape)
        image_rec1 = image_rec1.reshape(794, )
        #if(i == 199):
            #show_digit(image_rec1[10:794].reshape(28, 28), "reconstructed image with w/o sampling")
            #show_digit(image_rec1[0:10].reshape(1, -1), "reconstructed labels with w/o sampling")
        #if( i >99):
            #store_labels[i-100]= image_rec1[0:10]
            #print("stored labels : ", store_labels)
            #print("index i : ", i)

        #print("new shape of of rec1", image_rec1.shape)
        a = np.argmax(image_rec1[0:10], axis=0)
        rec_backup = np.where(image_rec1 > 0.5, 1, 0)
   ...
   # print("total addition of labels ", a)
    #rect_label = np.where(a == 1,) #index of the label in the 10-pixel array
    #rect_label1 =  rect_label[0]
    #print the result of construction
    #a1 = rec_backup[0:10]
    a2 = mnist.test.labels[j]
    a3 = np.where(a2 == 1)
    a4 = list(a3)
   # print("reconstructed label  ",a)
   # print("a[0] ",a[0])

   # print("org label position a3" , a3[0])
    #print("recondtructed label ", rect_label[0])
    #print("rect label length ",len(rect_label))
    #if(len(rect_label) == 1 ):
    if(a == a3[0]):
        accu[0] = accu[0] + 1
